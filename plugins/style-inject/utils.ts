...
import type {
  BuildCSSInjectionConfiguration,
  CSSInjectionConfiguration,
  PluginConfiguration,
} from "./types";
    ...
    load(id: string) {
      if (id == cssInjectedByJsId) {
        ...
        if (injectCodeFunction) {
          return `(${injectCodeFunction})(${cssCode}, ${JSON.stringify({
            styleId,
            useStrictCSP,
          })})`;
        }
...
export function clearImportedCssViteMetadataFromBundle(
  bundle: OutputBundle,
